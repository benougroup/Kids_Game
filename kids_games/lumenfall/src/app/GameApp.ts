...
export class GameApp {
  ...
  private readonly update = (dtMs: number): void => {
    ...
    try {
      ...
      this.applyMovement(polled.moveDx, polled.moveDy, dtMs, tx);
  ...
  private applyMovement(dx: number, dy: number, dtMs: number, tx: ReturnType<StateStore['beginTx']>): void {
    const runtime = tx.draftState.runtime;

    if (runtime.mode === 'EXPLORE' && (dx !== 0 || dy !== 0)) {
      tx.touchRuntimePlayer();
      runtime.player.x = Math.max(0, Math.min(WORLD_TILE_WIDTH - 1, runtime.player.x + dx));
      runtime.player.y = Math.max(0, Math.min(WORLD_TILE_HEIGHT - 1, runtime.player.y + dy));

      if (dx > 0) runtime.player.facing = 'right';
      if (dx < 0) runtime.player.facing = 'left';
      if (dy > 0) runtime.player.facing = 'down';
      if (dy < 0) runtime.player.facing = 'up';
    }

    tx.touchRuntimePlayer();
    const lerp = Math.min(1, dtMs / 1000 * 14);
    const targetPx = runtime.player.x * TILE_SIZE;
    const targetPy = runtime.player.y * TILE_SIZE;
    runtime.player.px += (targetPx - runtime.player.px) * lerp;
    runtime.player.py += (targetPy - runtime.player.py) * lerp;
  }
}
